...
+ EIGRP sử dụng thuật toán cập nhật khuếch tán (DUAL) để tính toán đường đi tốt nhất đến đích. Route tốt nhất được xác định là route có metric thấp nhất. Metric có thể được tính bằng các giá trị nư băng thông, độ trì hoãn của đường truyền, MTU(Maximum Transmission Unit). EIGRP mặc định dùng băng thông và độ trì hoãn để tính toán giá trị metric.
