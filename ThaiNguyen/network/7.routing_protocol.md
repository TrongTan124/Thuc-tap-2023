...
Số hop được xác định là số lượng bộ định tuyến xảy ra giữa nguồn và đích. Route có số hop thấp nhất được coi là route tốt nhất và được đưa vào bảng định tuyến(routing table). Router sẽ chọn quãng đường có số hop thấp nhất trong routing table để truyền các gói tin đi. Số hop tối đa cho cơ chế RIP là 15, nếu một địa chỉ có số hop 16 hoặc lớn hơn thì được coi là không thể truy cập.
 RIP exchanges updates of the network periodically between routers, and updates are always broadcasted in the network. Full routing tables are sent in updates. Routers always trust routing information received from neighbor routers. 
