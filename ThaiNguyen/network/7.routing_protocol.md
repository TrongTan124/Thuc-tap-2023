...
### How it works
- OSPF establish adjacencies with neighboring routers by exchanging Hello packets. Routers use Hello packets to discover and maintain neighbor relationships
- Each OSPF router collects information about its directly connected links, such as link status, cost, and priority. This information is stored in a Link-State Database (LSDB). OSPF routers then generate Link-State Advertisements (LSAs) containing information about their local links.
- OSPF routers flood their LSAs to all other routers within the OSPF area. Flooding ensures that all routers within the area have a consistent view of the network topology. 
- OSPF uses the Dijkstra's Shortest Path First (SPF) algorithm to calculate the shortest path to each network within the OSPF domain. Each router builds a Shortest Path Tree (SPT) based on the LSDB, determining the shortest paths to all destinations. The shortest path is calculated based on the accumulated cost or metric associated with each link.
