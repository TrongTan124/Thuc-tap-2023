...
- Nếu thời gian lease duration kết thúc và máy vẫn cần sử dụng địa chỉ IP, nó sẽ gửi một request đến DHCP server để gia hạn lease duration. Nếu DHCP server chấp nhận request này, nó sẽ gửi một acknowledgement message cho máy trên để xác nhận việc gia hạn. Lease duration sau đó sẽ được gia hạn.
